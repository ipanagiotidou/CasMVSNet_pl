...
class CascadeMVSNet(nn.Module):
    ...
    def __init__(self, n_depths=[8, 32, 48],
        ...
        for l in range(self.levels):
            ...
            setattr(self, f'cost_reg_{l}', cost_reg_l)    # I: και εδώ θέτει ως attribute του CascadeMVSNet τα 'cost_reg_0', 'cost_reg_1', και 'cost_reg_2'. Later, retrieves it with getattr(self, f'cost_reg_{l}') και το πασάρει στο def predict_depth.
            ...
            if l == self.levels-1: # coarsest level
                ...
                if isinstance(init_depth_min, float):
                    depth_values = init_depth_min + depth_interval_l * \
                                   torch.arange(0, D,
                                                device=imgs.device,
                                                dtype=imgs.dtype) # (D)    # I: returns a 1-D tensor with values from the interval [start, end) taken with common difference step. Here the step is the depth_interval_l. We offset with the init_depth_min.
